...
type browser interface {
	...
	Focus()
	SetDefaultBackgroundColor(r, g, b, a uint8) error
}
...
type WindowOptions struct {
	...
	IconId uint
	Center bool
	// Borderless creates a borderless window (WS_POPUP) that is suitable
	// for transparent overlays — use together with SetDefaultBackgroundColor
	// to show the desktop through the WebView2 control.
	Borderless bool
}
...
func wndproc(hwnd, msg, wp, lp uintptr) uintptr {
	if w, ok := getWindowContext(hwnd).(*webview); ok {
		...
		case w32.WMEraseBkgnd:
			// Return 1 (TRUE) to indicate the background was erased.
			// Prevents default white fill for borderless layered windows.
			return 1
		case w32.WMClose:
...
func (w *webview) CreateWithOptions(opts WindowOptions) bool {
	...
	exStyle := uintptr(0)
	if opts.Borderless {
		windowStyle = w32.WSPopup
		// WS_EX_NOREDIRECTIONBITMAP is the official Microsoft-recommended
	...
	// windows: a 1px border line, rounded corners, and drop shadow.
	// DWMWA_NCRENDERING_POLICY → DWMNCRP_DISABLED: disables DWM's non-client
	// rendering entirely — no border, no shadow, no rounded corners.
	// DWMWA_BORDER_COLOR → DWMWA_COLOR_NONE: explicitly tells DWM to draw
	// no border line, in case the rendering policy alone doesn't cover it.
	if opts.Borderless {
		w32.DwmSetWindowAttributeInt(w.hwnd, w32.DWMWA_NCRENDERING_POLICY, w32.DWMNCRP_DISABLED)
		w32.DwmSetWindowAttributeInt(w.hwnd, w32.DWMWA_BORDER_COLOR, w32.DWMWA_COLOR_NONE)
	}
...
// SetDefaultBackgroundColor delegates to the Chromium browser's
// SetDefaultBackgroundColor. A fully transparent background (a == 0) makes
// the WebView2 control transparent so the desktop can show through a
// layered host window.
